...
def final_test(checkpoint_path,saving_path, args):
    ...
    all_preds = pd.DataFrame(columns=["thresh", "AUROC", "Accu", "Sens", "Spec", "f1","f1_weighted","Acc"])
    ...
    for thresh in thre:
        ...
        AUROCs,accuracy, Accus, Senss, Specs, pre, f1,f1_weighted = epochVal_metrics_test(
        ...
        f1_weighted_avg = np.array(f1_weighted).mean()
        ...
        print(f"f1_weigh_avg: {f1_weighted_avg}")
    ...
    all_preds.to_csv(saving_path, index=False)
